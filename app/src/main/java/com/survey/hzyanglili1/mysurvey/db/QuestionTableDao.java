...
public class QuestionTableDao {
    ...
    public Question cursor2Ques(Cursor cursor){
        ...
        int quesId = cursor.getInt(cursor.getColumnIndex("question_id"));
        String quesTitle = cursor.getString(cursor.getColumnIndex("question_title"));
        String quesImage = cursor.getString(cursor.getColumnIndex("question_image"));
        String optionText = cursor.getString(cursor.getColumnIndex("option_text"));
        String optionImage = cursor.getString(cursor.getColumnIndex("option_image"));
        Boolean isMust = (cursor.getInt(cursor.getColumnIndex("qustion_ismust")) == 0)?false:true;
        Boolean isMulti = (cursor.getInt(cursor.getColumnIndex("question_ismulti")) == 0)?false:true;

        Question question = null;

        switch (type){
            case 1://选择
                question = new XuanZeQuestion(Integer.parseInt(surveyId),quesId,quesTitle,quesImage,isMust,isMulti,optionText,optionImage);
                break;
            case 2://填空
                question = new TiankongQuestion(Integer.parseInt(surveyId),quesId,quesTitle,quesImage,isMust);
                break;
            case 3://程度
                question = new ChengduQuestion(Integer.parseInt(surveyId),quesId,quesTitle,quesImage,optionText,isMust);
                break;
